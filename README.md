...
```
ASCIIShooter/
├── ASCIIShooter.sln          ← Open this in VS 2022
│
...
│
└── ASCIIShooter/             ← Executable: wires everything together
...
- **Wall edge detection**: dot-product corner test → renders `|` borders
- **Ceiling/floor**: gradient ASCII patterns (`-`, `.`, `#`, `x`, ` `)
- **Sprite pass**: enemies sorted far→near, depth-tested against `depthBuffer`
...
### Steps
1. Open `ASCIIShooter.sln` in Visual Studio 2022.
2. Select **Debug | x64** or **Release | x64**.
3. Build → **Ctrl+Shift+B** (or Build → Build Solution).
4. Run → **Ctrl+F5** (without debugger, so the console stays open).

Output: `bin\Debug\ASCIIShooter.exe` or `bin\Release\ASCIIShooter.exe`.

> **Tip:** Run from a native Windows Terminal or `cmd.exe` for best rendering.
> The console window will auto-resize to 120×40. If text appears garbled,
> set the console font to **Consolas** or **Lucida Console** at 14pt.
